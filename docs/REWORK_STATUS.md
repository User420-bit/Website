...
| 4.3 | E-Mail-Adresse bestätigen                                                                    | Erledigt: Die Fallback-Adresse steht in `company.json` und `legal.json` einheitlich auf `klartextpsd.info@gmail.com`. **[Sicher]** von Pharrel bestätigt.                                                                |
| 4.4 | Pro Projekt: Screenshots, echtes Code-Beispiel, ein Satz "was würde ich heute anders machen" | Schema und Rendering stehen (`screenshots`, `codeExample`, `learnings`). Ein echtes Code-Beispiel steht noch in keinem Projekt.                                                                                          |
