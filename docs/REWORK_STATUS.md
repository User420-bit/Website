...
| 2. Interne Navigation kaputt               | **behoben**                | Klick im Footer auf "Impressum" → URL `/Website/impressum/`, H1 "Impressum". Deep-Link auf eine Projektseite → H1 mit dem Projekttitel, ohne Redirect.                                                                  |
...
| Toter GitHub-Link (Plan: [Wahrscheinlich]) | **bestätigt: [Sicher]**    | Die GitHub-Nutzersuche nach `pharrelsandjo` liefert 0 Treffer. Der Nutzer existiert nicht, also sind **beide** Links tot — Profil und ein Projekt-Repository. Beide stehen jetzt auf `null` und werden nicht gerendert. |
...
| 4.3 | Korrekte GitHub-URL                                                                          | `pharrelsandjo` existiert nicht. `github` steht in `profile.json` und in der betroffenen Projektdatei auf `null` — ein toter Link ist schlechter als kein Link.                                                                                                                                                                                                                                                                                                                                                                                            |
| 4.3 | E-Mail-Adresse bestätigen                                                                    | `studsandph@gmail.com` steht unverändert aus dem alten Content. **[Vermutung]**, ob sie stimmt.                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 4.4 | Pro Projekt: Screenshots, echtes Code-Beispiel, ein Satz "was würde ich heute anders machen" | Schema und Rendering stehen (`screenshots`, `codeExample`, `learnings`). Screenshots seit 2026-09-23 per `npm run screenshots` (`scripts/screenshots.mjs`) für TIEFGANG, DealerSim und MemoryTree, dazu Handaufnahmen aus beiden Spielen und aus MemoryTree, auch im Handyformat. PointCare ist zurückgestellt, bis die Seite abgenommen ist (der Entwurf zeigt noch Platzhalter), JustBeauty bis zur Zusage des Kunden; Kleinkram, NoteList, Feynman und Availably haben keine erreichbare Quelle. Ein echtes Code-Beispiel steht noch in keinem Projekt. |
