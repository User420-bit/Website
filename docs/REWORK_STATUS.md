...
| 4.4 | Pro Projekt: Screenshots, echtes Code-Beispiel, ein Satz "was würde ich heute anders machen" | Schema und Rendering stehen (`screenshots`, `codeExample`, `learnings`). Screenshots seit 2026-09-23 per `npm run screenshots` (`scripts/screenshots.mjs`) für PointCare, TIEFGANG, DealerSim und MemoryTree; JustBeauty, Kleinkram, NoteList, Feynman und Availably haben keine erreichbare Quelle. Ein echtes Code-Beispiel steht noch in keinem Projekt. |
