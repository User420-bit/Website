...
1. **[Sicher] Die Seite ist vermutlich nie live gegangen.** Der einzige Lauf des Deploy-Workflows (Run #1 vom 2026-04-29) ist im Job `build` fehlgeschlagen. Die Logs sind inzwischen abgelaufen (HTTP 410), die Ursache lässt sich nur durch einen erneuten Lauf feststellen. Bis das geklärt ist, ist jede Design-Diskussion zweitrangig.
...
Phase 0.1: Workflow `Deploy to GitHub Pages` per `workflow_dispatch` starten, Logs sichern, Ursache beheben. Erst wenn die Pages-URL antwortet, beginnt Phase 0.2.
